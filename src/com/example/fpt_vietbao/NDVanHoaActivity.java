...
@SuppressLint("NewApi")
public class NDVanHoaActivity extends Activity {
	// khai bao
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (check.equals("tiengviet")) {
			getActionBar().setTitle("Văn Hóa");
		} else {
			getActionBar().setTitle("Cultural");
		}
		...
		adapter2 = new SimpleAdapter(NDVanHoaActivity.this, array,
				R.layout.custom_tinnong, from, to);
		...
		// doc theme
				...
				if (check.equals("bandem")) {
					...
					NDVanHoaActivity.this.setTheme(android.R.style.Theme_DeviceDefault);
				} else {
	...
	public void redDataofTable() {
		...
		array.clear();
		arrLink.clear();
		Cursor c = database.query("tbVanHoa", null, null, null, null, null,
				null);
		c.moveToFirst();
		while (c.isAfterLast() == false) {
