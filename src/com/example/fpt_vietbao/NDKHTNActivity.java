...
@SuppressLint("NewApi")
public class NDKHTNActivity extends Activity {
	// khai bao
...
protected void onCreate(Bundle savedInstanceState) {
	...
	// luu ngon ngu vao dât
			...
			if (check.equals("tiengviet")) {
				getActionBar().setTitle("KH-TN");
			} else {
				getActionBar().setTitle("Natural Sciences");
			}
	...
	adapter2 = new SimpleAdapter(NDKHTNActivity.this,array,R.layout.custom_tinnong,from,to);
	...
	// doc theme
			...
			if (check.equals("bandem")) {
				...
				NDKHTNActivity.this.setTheme(android.R.style.Theme_DeviceDefault);
			} else {
...
public void redDataofTable() {
	...
	array.clear();
	arrLink.clear();
	Cursor c = database.query("tbKHTN", null, null, null, null, null, null);
	c.moveToFirst();
	while (c.isAfterLast() == false) {
