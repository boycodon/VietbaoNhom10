...
@SuppressLint("NewApi")
public class NDBatDonSanActivity extends Activity {
	// khai bao
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (check.equals("tiengviet")) {
			getActionBar().setTitle("Bất Động Sản");
		} else {
			getActionBar().setTitle("Realty");
		}
		...
		adapter2 = new SimpleAdapter(NDBatDonSanActivity.this, array,R.layout.custom_tinnong, from, to);
		...
		if (check.equals("bandem")) {
			...
			NDBatDonSanActivity.this.setTheme(android.R.style.Theme_DeviceDefault);
		} else {
	...
	public void redDataofTable() {
		...
		array.clear();
		arrLink.clear();
		Cursor c = database.query("tbBatDongSan", null, null, null, null, null,
				null);
		c.moveToFirst();
		while (c.isAfterLast() == false) {
